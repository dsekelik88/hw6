...
window.addEventListener('DOMContentLoaded', async function(event) {
    ...
    let url = 'https://api.themoviedb.org/3/movie/now_playing?api_key=149a32d09f374e3c2dfbd67ece962ed8&language=en-US'
    ...
    let queryMovieList = await db.collection('mov').get()
    let movIDs = queryMovieList.docs
    let watchedArray = []

    for (let i=0; i<movIDs.length;i++) {
        let record = movIDs[i].data()
        let movID = record.title
        // console.log(record.title)
        watchedArray.push(movID)
    }
    ...
    async function renderMovie(movieID, posterURL) {
        document.querySelector('.movies').insertAdjacentHTML('beforeend', `        
            <div class="mov-${movieID} w-1/5 p-4 movie-abcdefg1234567">
        ...
        document.querySelector(`.mov-${movieID} .watched-button`).addEventListener('click', async function(event) {
            ...
            console.log(`movie ${movieID} marked played!`)
            document.querySelector(`.mov-${movieID}`).classList.add('opacity-20')
